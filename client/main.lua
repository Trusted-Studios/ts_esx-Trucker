...
function Info:FinishJob(info, reward, x, y, z)
    ...
    if GetEntityModel(veh) == GetHashKey(currentVehicle) then 
        ...
        DoScreenFadeIn(1000)
        Citizen.Wait(800)
        --StartMission("~y~Auftrag erfolgreich abgeschlossen!", "Du hast für deine Arbeit ~g~$"..reward.."~w~ bekommen.")
    else
        DoScreenFadeOut(1000)
        Citizen.Wait(1200)
        if (DoesEntityExist(ped) and not IsEntityDead(ped)) then 
        ...
        DoScreenFadeIn(1000)
        Citizen.Wait(1200)
        --MissionFailed("Auftrag fehlgeschlagen!", "Du bist mit dem falschen Truck zurück gefahren!")
